...
## Terrain, grid and resource review (2026-09-11). **Status: Implementation started — the two high-severity items (DUP-14, FIX-07) are landed; 15 remain.**
...
| FIX-01 | [Hillslope diffusion coefficient exceeds its documented stability limit at high ErosionStrength](../plans/terrain-grid/FIX-01-erosion-diffusion-stability.md) | fix | XS | Proposed |
| FIX-02 | [Shape/ShapeWarpX/ShapeWarpY/Detail noise channels are built every generation but read by nothing](../plans/terrain-grid/FIX-02-dead-noise-channels.md) | fix (owner-call removal) | XS | Proposed |
| FIX-03 | [TerrainTextures.Load throws NRE on a failed external-file load instead of returning null](../plans/terrain-grid/FIX-03-terraintextures-load-nre.md) | fix | XS | Proposed |
...
| FIX-11 | [GridCalendarHudComponent wires calendar signals only in _Ready and never reconnects on re-resolve](../plans/terrain-grid/FIX-11-calendar-hud-signal-reconnect.md) | fix | S | Proposed |
| FIX-12 | [GridPorts.Transfer silently discards the un-accepted remainder for an unload-only giver (breaks its never-lost contract)](../plans/terrain-grid/FIX-12-ports-transfer-remainder.md) | fix | XS | Proposed |
| FIX-13 | [A rejected actor-travel restore loads nothing and reports nothing](../plans/terrain-grid/FIX-13-actor-travel-restore-report.md) | fix | XS | Proposed |

Suggested order (progress 2026-09-11): the two high-severity data-integrity items **DUP-14 and FIX-07 are DONE** (each landed with a mutation-proven guard). Next: the rest of the load/restore family (FIX-05/06/09/11/13), then the standalone terrain fixes (FIX-01/03/04) and the XS grid fixes (FIX-08, FIX-10, FIX-12), with the two removals/enhancements (FIX-02, ENH-17) and the two refactors (DUP-15, DUP-16) as they fit. FIX-02 is an owner's-call removal. When an item lands: set its status here and in `plans/terrain-grid/README.md`, and move its evidence into the component pages under `docs/terrain-engine/` and `docs/grid-system/`.
