...
$sections = @(
    ...
    New-Section "terrain" "2D Grid, Terrain, Builder Systems" @("ecs/terrain") "Painterly terrain, grid projection, tilemap bridging, selection, placement, roads, navigation, path following, job queues, resources, workers, spawners, and builder HUD/resource panels." "Use this for top-down, isometric, city-builder, RTS, farming, survival, and Settlers-style workflows." "Build the world at design time with authored Node2D/Control nodes, then attach grid components. Prefer painterly terrain or sparse visual TileMap layers over huge fully painted maps."
...
[void]$md.AppendLine("4. For grid games, use PainterlyTerrainComponent plus GridProjectionComponent, GridCellDataComponent, GridPlacementComponent, GridNavigationComponent, GridJobQueueComponent, and GridWorkerSpawnerComponent as separate nodes.")
...
[void]$md.AppendLine()
[void]$md.AppendLine("- tests/examples/grid_world_kit_hud_example.tscn: playable Settlers-style builder slice using terrain, resources, tools, workers, and kit HUD.")
[void]$md.AppendLine("- tests/examples/base_worker_templates_example.tscn: focused depot/worker template scene.")
[void]$md.AppendLine()
...
[void]$md.AppendLine("- powershell -ExecutionPolicy Bypass -File tests/render_scene_probe.ps1 -GodotCommand H:/dev/Godot/Godot_v4.7-stable_mono_win64.exe -ScenePath res://tests/examples/grid_world_kit_hud_example.tscn -TimeoutSeconds 90")
