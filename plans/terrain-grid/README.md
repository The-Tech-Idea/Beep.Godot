...
| ENH-12 | [Job queue indices](ENH-12-job-queue-indices.md) | bucketed states, priority-heap claim, one notification, typed enumeration | M | **partial** (one-notification: pins refresh once per mutation; buckets + claim-index + typed-enum pending) |
