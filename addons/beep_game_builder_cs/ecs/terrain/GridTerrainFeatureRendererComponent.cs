...
namespace Beep.ECS
{
    ...
    [Tool]
    [GlobalClass]
    public partial class GridTerrainFeatureRendererComponent : Node2D
    {
        ...
        public void Rebuild()
        {
            ...
            for (int y = 0; y < size.Y; y++)
            {
                ...
                {
                    string feature = _generator.FeatureAt(new Vector2I(x, y));
                    if (feature.Length == 0)
                        continue;
        ...
        private void AddStamp(Texture2D sheet, int columns, int rows, int x, int y, float tile, int slot)
        {
            ...
            int index = Mathf.FloorToInt(Hash01(x, y, Seed + 811 + (slot * 97)) * columns * rows) % (columns * rows);
            ...
            float jitterScale = 1.0f + ((Hash01(x, y, Seed + 907 + (slot * 89)) - 0.5f) * 2.0f * ScaleJitter);
            Vector2 drawn = (Vector2)frame * fit * SpriteScale * jitterScale;

            var centre = new Vector2(
                (x + 0.5f + ((Hash01(x, y, Seed + 1013 + (slot * 71)) - 0.5f) * PositionJitter)) * tile,
                // Nudged up so the trunk sits at the tile centre and the canopy
                // overhangs upward, which is how the eye reads depth.
                (y + 0.42f + ((Hash01(x, y, Seed + 1117 + (slot * 67)) - 0.5f) * PositionJitter)) * tile);
