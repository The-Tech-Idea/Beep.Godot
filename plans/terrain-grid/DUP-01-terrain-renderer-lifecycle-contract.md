...
- **`TerrainWorldComponent.Draw()` is untouched.** Rewriting its per-renderer wiring as a `ShownIn`-filtered loop is step 3, it collides with the level/campaign session editing `TerrainWorldComponent`, and it is not a duplication fix but a redesign of behaviour-sensitive orchestration (the visibility invariants that method's own comment warns about). It waits for coordination.
