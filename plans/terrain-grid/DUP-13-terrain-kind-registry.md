...
- **Coherence flags** (done — TerrainCoherenceStage): `Absorbable`, `AbsorbTarget`, `PeakMaterial` folded in (steps 2–3); **Rainfall** = {desert,dry_grass,grass,swamp,jungle} folded onto `TerrainKind.Rainfall`. The "entangled" ordered voting index turned out order-INDEPENDENT: `Smooth` encodes each kind as a small byte for a compact vote and inverts via `RainfallKinds[best-1]`, but the winner is decided by neighbour counts and scan order, not the index, so it round-trips. Catalog lists them in catalog order (grass,dry_grass,desert,jungle,swamp) and the terrain layer is byte-identical - the proof. Baseline-guarded (hashed layer) + per-kind probe assertions; order deliberately NOT asserted. Mutation-proven (grass true→false trips both).
...
- **FeatureEligibility** (done — TerrainFeatureStage): folded onto `TerrainKind.FeatureEligibility` ("jungle"/"marsh"/"oasis"/"woods"/""). Both the ranking pass and `Choose` read it, so the two woods-capable tests can't drift. The oasis roll, temperature floor, moisture ranking, Forest-vs-Woods split and tundra's -0.05 stand bias all stay in the stage. Feature is a hashed layer, so the baseline is the primary guard; the catalog probe adds a per-kind assertion. Mutation-proven (grass "woods"→"" trips both).
...
- **IsoFrame** (TerrainIsometricRenderer) — **DECIDED 2026-09-10: leave the exports as-is; intentionally NOT a catalog table.** Triage: these are 11 per-scene `[Export] int XFrame` primary frames (grass0…rock10) PLUS a `TerrainVariants` string[] override; mud and lava have no export and are assembled in code (mud=swamp's frame, lava=rock's), deep/shallow_water have no block frame (surface shader). Unlike MaterialSlot (a private static table with no override), this is an authoring surface: a game rearranges its own atlas through those exports. Folding it would mean either (a) catalog default + export override with no "unset" sentinel — an int export always has a value, so the renderer cannot tell "use my default" from "use the catalog", a second owner, not a resolution; or (b) retiring the 11 exports — an authoring/removal change. It is not a duplicate of the catalog (the iso atlas layout is genuinely independent) and not a missing implementation (it works). Fahad chose to keep the exports as the per-scene authoring surface, so DUP-13 does not claim this table.
...
**The biome-preset stage stays out of the catalog (analysed 2026-09-11).** `TerrainBiomeStage` is not a per-kind classification table like the others — it is the one stage that ASSIGNS a cell's kind, the SOURCE the catalog is downstream of. Its functions produce kinds; they do not classify them:
- `PlainGround(preset)` and `ThemedKind(preset, elevation)` are keyed on `TerrainPreset`, not on a kind, and the relation is many-to-many (Desert preset → desert OR rock by elevation; `rock` is produced by Desert/Sand/Rock/Snow/Lava presets). That cannot be a per-kind `TerrainKind` property — there is no single kind to hang it on.
- `LandKind`/`EarlyKind`/`WaterKind` compute the kind from climate (Whittaker moisture bands, temperature floors, shelf distance) — generator tuning, not a fact about a kind.
...
| `TerrainBiomeStage.ThemedKind`/`PlainGround`/`EarlyKind` | now ~135-163 | preset ground kinds — **stays: assignment source, not a per-kind table (see Progress)** |
