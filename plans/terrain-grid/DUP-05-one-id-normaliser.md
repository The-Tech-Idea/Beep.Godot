...
**Outcome: the node-name sanitiser (2026-09-09).** `GridIds.NodeName(value, fallback)` is the one owner now: `NormalizeOr` (so a node named after an id matches the normalised id it is built from), then every `Path.GetInvalidFileNameChars()` plus `'/'`, `'\'` and `':'` to `'_'`. The five copies are gone - the two lower-casing ones (`GridResourceScatterComponent`, `GridWorkerSpawnerComponent`), the `protected` base on `GridPanelComponent`, and the two forwarders (`GridBuildToolbarComponent`, `GridResourceBarComponent`) - across nine call sites, plus `GridListPanelComponent` which had called the base directly. The one case rule chosen is Normalize's (lower-case, `'-'`->`'_'`), which the id normaliser change already established is safe here: no test or scene references a generated node name by literal string (re-checked), and a node name that tracks its normalised id is the consistent choice. A scan pin forbids any `SafeName(` definition under `ecs/grid`/`ecs/terrain` and requires `GridIds.NodeName`; mutation-proven. The worker's required-members pin, which had asserted the literal `SafeName(WorkerIdPrefix)` sanitise call, now asserts `GridIds.NodeName(WorkerIdPrefix`. Build clean; grid-ids, lake-scatter, worker-dispatch, live-resource-view and building probes green.
