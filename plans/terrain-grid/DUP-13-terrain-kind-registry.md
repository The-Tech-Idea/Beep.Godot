...
Suggested remaining order (least entangled first); **done: Startable, coherence+scale material flags, Level**. Remaining: Class (GroundOf/IsWaterKind — wide usage, water aliases; a `Standard`-read migration guarded by the round-trip/grid tests since generation may hash it) → BlockedByDefault (GridTerrainRules — water/sea/ocean aliases) → PropPalette (aliases + the AllowShallowWaterProps conditional) → MaterialSlot/IsoFrame (per-renderer `[Export]` **defaults**, not replacements — guarded by `terrain_kind_catalog_probe`/render probes, not the baseline) → FeatureEligibility (jungle/marsh/oasis/woods; Forest-vs-Woods thresholds and tundra's -0.05 bias stay in the stage) → the Rainfall ordered voting index (the entangled one) → the biome-preset stage (a biome→kind map, not a per-kind property — needs a catalog method or stays) → delete the literals + add the no-literals pin → game-assignable catalog through settings/recipe. View-only migrations (Level and everything after MaterialSlot) extend `terrain_kind_catalog_probe`; generation migrations extend the baseline.
