...
| ENH-12 | [Job queue indices](ENH-12-job-queue-indices.md) | bucketed states, priority-heap claim, one notification, typed enumeration | M | **partial** (one-notification + typed HUD enum + fairness baseline done; bucketed counts + spatial claim-index pending) |
