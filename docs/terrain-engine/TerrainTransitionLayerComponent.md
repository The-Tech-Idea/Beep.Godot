...
- `void RefreshTransitions()` — the main entry point: resolves node references, places the display layer(s) at the correct z-index (`PlaceDisplayLayer`), builds the `TileSet`(s) from atlas paths if configured (manual path only), then either delegates to `RefreshUsingTileSetTerrains()` (Godot terrain-connect path) or repaints every cell itself via `DualGridMaskAt`/`AtlasCoordinatesForMask` (manual path). Warns and returns early if the resolved display `TileSet` is null. Before painting, it erases the cells it painted on its previous refresh that the new bounds no longer cover (bounds plus one row and column on the manual path, bounds plus one ring on the terrain-set path), so a map shrunk or moved in place leaves no strips of the old one. Cells it never painted, such as authored cells on a shared display, are left alone.
