...
`GridCalendarComponent` tracks days, seasons, and years and ticks crop growth in the cell data. It does not run a clock of its own: `GridWorkClockComponent` advances it one day at a time, from the game clock's cascade when a game is running (one end-turn is one day in a turn-based game; a day passes smoothly in a real-time one) or at one turn per second when a grid scene runs standalone. `GridCalendarHudComponent` shows the date, the day's progress (read from the work clock), and an optional next-day button.
...
sequenceDiagram
    ...
    W->>W: work for WorkTurns (on the work clock)
