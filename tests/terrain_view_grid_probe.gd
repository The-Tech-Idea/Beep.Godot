...
func run() -> void:
	...
	# FEAT-09: with a start area component the framing opens on ITS active start, not start 0.
	# The fixture's Mode 0 generates no starts; the generator's default mode does.
	var defaults = GENERATOR.new()
	generator.set("Mode", defaults.get("Mode"))
	defaults.free()
	generator.set("StartPositionCount", 4)
	var framed_starts: Array = generator.call("GetStartPositions")
	check(framed_starts.size() >= 2, "the fixture map needs two starts to tell start framing apart (%d)" % framed_starts.size())
	var start_area = load("res://addons/beep_game_builder_cs/ecs/grid/GridStartAreaComponent.cs").new()
	start_area.set("LocalStartIndex", framed_starts.size() - 1)
	root.add_child(start_area)
	framing.set("StartAreaPath", framing.get_path_to(start_area))
	framing.call("FrameStartPosition")
	var active_start: Vector2 = world.call("StartPositionGlobalAt", framed_starts.size() - 1)
	check(active_start.distance_to(world.call("StartPositionGlobal")) > 1.0 and camera.global_position.distance_to(active_start) < 0.01,
		"Camera framed start 0 instead of the start area's active start")
	framing.set("StartAreaPath", NodePath(""))
	start_area.free()
