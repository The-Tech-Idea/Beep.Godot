...
- [x] **Phase 5 — Archetypes per genre** — **done.** **The "must not" is now checkable:** `validate_scenes.sh`
      ...
      **`docs/ARCHETYPES.md` written** — the developer-facing reference (per-archetype required/
      optional/must-not + node types + the mobile-interactable split + the zero-archetype genres),
