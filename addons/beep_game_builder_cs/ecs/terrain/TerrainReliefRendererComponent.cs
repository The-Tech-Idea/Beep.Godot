...
namespace Beep.ECS
{
    ...
    public partial class TerrainReliefRendererComponent : Node2D
    {
        ...
        public void Rebuild()
        {
            ...
            {
                GD.PushWarning($"[{Name}] no generator at TerrainGeneratorPath; no relief was drawn.");
                QueueRedraw();
                return;
            }
            ...
            for (int y = 0; y < size.Y; y++)
            {
                ...
                {
                    int relief = (int)field.ReliefAtCell(new Vector2I(x, y));
            ...
            Texture2D sheet, int columns, int rows, int x, int y, float tile, int slot, float scale)
        {
            ...
            var centre = new Vector2(
                (x + 0.5f + ((TerrainGeometry.Hash01(x, y, Seed + 4357 + (slot * 71)) - 0.5f) * PositionJitter)) * tile,
                // Nudged up so the base sits at the tile centre and the mass
                // rises above it, which is how the eye reads elevation.
                (y + 0.40f + ((TerrainGeometry.Hash01(x, y, Seed + 4463 + (slot * 67)) - 0.5f) * PositionJitter)) * tile);
