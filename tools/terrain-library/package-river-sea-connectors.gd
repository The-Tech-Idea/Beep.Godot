...
const BASE = "res://addons/beep_game_builder_cs/generated/dev/cartoon/water/river_sea_connectors_v1/"
...
func run() -> void:
	...
	for projection in ["square","isometric"]:
		...
		var scene = load(SEA+projection+"/sea_review.tscn").instantiate()
		...
		var texture = ImageTexture.create_from_image(Image.load_from_file(folder+"river_sea_16.png"))
		check(ResourceSaver.save(texture,folder+"river_sea_16.res")==OK,"Save connector texture")
		var atlas = TileSetAtlasSource.new()
		atlas.texture = load(folder+"river_sea_16.res")
		...
		check(ResourceSaver.save(tiles,folder+"river_sea.tres")==OK,"Save estuary TileSet")
		layer.tile_set = load(folder+"river_sea.tres")
		layer.clear()
		for y in range(11):
			for x in range(11): layer.set_cell(Vector2i(x,y),0,Vector2i(7,5),0)
		var cells: Array[Vector2i] = []
		# Water extends beyond the visible coast; no artificial enclosed lake boundary.
		for y in range(3,14):
			for x in range(-3,14): cells.append(Vector2i(x,y))
		...
		for cell in layer.get_used_cells(): check(layer.get_cell_tile_data(cell)!=null,"Missing estuary tile")
		scene.name = "ExplicitRiverSeaInletReview"
		var packed = PackedScene.new()
		check(packed.pack(scene)==OK,"Pack estuary scene")
		check(ResourceSaver.save(packed,folder+"river_sea_review.tscn")==OK,"Save estuary scene")
		scene.free()
	print("RIVER SEA ","PASSED" if errors.is_empty() else "FAILED")
