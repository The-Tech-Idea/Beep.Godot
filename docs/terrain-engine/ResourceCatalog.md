...
- **Update (VIEW-13, 2026-09-15):** the overlay's category-coloured resource discs were removed (the icon renderer is the one resource drawer), so no presentation code maps an id to a category today. Both `CategoryOf` methods are now unread and marked in place, with ENH-13's minimap resource tints named as the intended consumer. The note below records the state before that change.
