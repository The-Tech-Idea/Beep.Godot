...
$startInfo.Arguments = "--headless --audio-driver Dummy --path `"$($projectRoot.Path)`" --script res://tests/grid_job_queue_probe.gd"
...
if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "Godot grid job queue probe timed out after $TimeoutSeconds seconds."
}
