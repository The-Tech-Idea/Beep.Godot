...
**Follow-up, 2026-09-08 (ENH-16).** The iterator is gone: `TerrainGeometry.Neighbours4` fills a caller's four-int span in the iterator's order (-x, +x, -y, +y) and serves its four BFS sites plus `ClearOrphanWaterSamples`, which already walked that way - bit-identical, confirmed by the recorded field snapshot. The five unrolled loops in the other order (+x, -x, +y, -y) remain: `Landmass.Grow`, `Erosion.Diffuse`, `Coherence.AbsorbOnce`, `ScaleConstraint.NeighbourLand`, `ScaleConstraint.Regions`, and the iso renderer's copy. They land in their own commit with the snapshot re-recorded and the output change quantified.
