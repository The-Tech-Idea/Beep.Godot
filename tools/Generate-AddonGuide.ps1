...
$sections = @(
    ...
    New-Section "terrain" "2D Grid, Terrain, Builder Systems" @("ecs/terrain") "A seeded terrain engine with painted, tile and isometric views, plus grid projection, tilemap bridging, selection, placement, roads, navigation, path following, job queues, resources, workers, spawners, and builder HUD/resource panels." "Use this for top-down, isometric, city-builder, RTS, farming, survival, and Settlers-style workflows." "Build the world at design time with authored Node2D/Control nodes, then attach grid components. Let TerrainWorldComponent create and draw the map - its Generate map button does so in the editor and the result is saved with the scene."
