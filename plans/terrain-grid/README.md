...
| ENH-12 | [Job queue indices](ENH-12-job-queue-indices.md) | bucketed states, priority-heap claim, one notification, typed enumeration | M | **mostly done** (one-notification + cached counts + typed HUD enum + fairness baseline; only the spatial nearest-job claim index remains) |
